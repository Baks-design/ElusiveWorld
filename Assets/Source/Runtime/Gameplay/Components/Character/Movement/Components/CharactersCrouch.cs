...
namespace ElusiveWorld.Core.Assets.Scripts.Behaviours.Characters
{
    public class CharactersCrouch
    {
        ...
        {
            if (input.IsCrouchHeld && checks.CanCrouch())
            {
                var canSlide =
        ...
        {
            if (flags.isCrouching) StopCrouchOrSlide();
        }
        ...
        void StartCrouch()
        {
            if (input.TryConsumeBufferedCrouch() && !flags.isCrouching)
            {
        ...
        {
            if (input.TryConsumeBufferedCrouch() && !checks.CheckIfRoof())
            {
                flags.isCrouching = false;
                flags.isSliding = false;
            }
