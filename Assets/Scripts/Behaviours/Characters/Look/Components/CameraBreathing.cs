...
namespace ElusiveWorld.Core.Assets.Scripts.Behaviours.Characters
{
    public class CameraBreathing
    {
        ...
        public CameraBreathing(CharactersLookFlags flags, LookSettings settings, Transform camTransform)
        {
        ...
        Vector3 GetPositionOffset(Vector3 noise) =>
            new(settings.x ? noise.x : 0f, settings.y ? noise.y : 0f, settings.z ? noise.z : 0f);

        Quaternion GetRotationOffset(Vector3 noise) =>
            Quaternion.Euler(settings.x ? noise.x : 0f, settings.y ? noise.y : 0f, settings.z ? noise.z : 0f);
    }
}