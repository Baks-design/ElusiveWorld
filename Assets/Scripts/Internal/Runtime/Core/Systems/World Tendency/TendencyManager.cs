...
{
    public class TendencyManager : Singleton<TendencyManager>, ITendencySystem //TODO: Apply Tendency
    {
        [Header("Settings")]
        ...
        Dictionary<string, float> regionalTendencies = new();
        readonly List<ITendencyEffect> activeEffects = new();

        public float GlobalTendency => globalTendency;

        public event Action<TendencyState> OnStateChanged;
        public event Action<string, float> OnActionRegistered;
        public event Action<TendencyState> OnTendencyChanged;
